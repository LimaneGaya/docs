...
import 'package:docs/components/document/state/document_state.dart';
...
class DocumentController extends StateNotifier<DocumentState> {
  ...
  Future<void> _setupDocument() async {
    try {
      final docPageData = await _read.read(Repository.database).getPage(
            documentId: state.id,
          );
  ...
  Future<void> _setupListeners() async {
    final subscription =
        _read.read(Repository.database).subscribeToPage(pageId: state.id);
  ...
  Future<void> _broadcastDeltaUpdate(Delta delta) async {
    _read.read(Repository.database).updateDelta(
          pageId: state.id,
          deltaData: DeltaData(
            user: _read.read(AppState.auth).user!.$id,
  ...
  Future<void> saveDocumentImmediately() async {
    ...
    try {
      await _read.read(Repository.database).updatePage(
            documentId: state.id,
